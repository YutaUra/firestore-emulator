...
beforeEach(async () => {
  ...
  await fetch(
    `http://localhost:8082/emulator/v1/projects/${process.env.GCLOUD_PROJECT}/databases/(default)/documents`,
    { method: "DELETE" },
  );
});
...
describe("create with same id", () => {
  it("simple document", async () => {
    ...
    expect(emulatorResult.reason).toStrictEqual(expect.objectContaining({
      code: realResult.reason.code,
      details: realResult.reason.details,
      note: realResult.reason.note,
    }));
  });
